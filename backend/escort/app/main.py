...
async def _handle_trip_offered(data: dict, producer: Producer) -> None:
    try:
        escort_id = uuid.UUID(str(data["escort_id"]))
        trip_id = uuid.UUID(str(data["trip_id"]))
    except (KeyError, ValueError):
        logger.warning("Malformed trip.offered.escort message: %s", data)
        return
    ...
    await loop.run_in_executor(
        None,
        lambda: (producer.produce("trip.notified.escort", value=payload), producer.flush()),
    )
    logger.info("Published trip.notified.escort for trip %s", trip_id)
...
async def _consumer_loop(stop_event: asyncio.Event) -> None:
    ...
    consumer = Consumer(
        _make_kafka_config({
            "group.id": "escort-service-group",
            "auto.offset.reset": "earliest",
        })
    )
    producer = Producer(_make_kafka_config())
    consumer.subscribe(["trip.offered.escort"])
    logger.info("Kafka consumer started on topic trip.offered.escort")
...
app = FastAPI(title="Escort Service", lifespan=lifespan)
app.include_router(router, prefix="/escorts")
